...
interface UseCampaignAgentCallReturn {
  isConnected: boolean;
  isRecording: boolean;
  connectionStatus: string;
  messages: Array<{ text: string; sender: 'user' | 'bot' }>;
  connectToAgent: (config: CampaignAgentConfig) => Promise<Room>;
  disconnectFromAgent: () => Promise<void>;
  addMessage: (text: string, sender: 'user' | 'bot') => void;
}

export const useCampaignAgentCall = (): UseCampaignAgentCallReturn => {
  ...
  const generateRoomName = useCallback(() => {
    return 'campaign-chat-' + Date.now() + '-' + Math.random().toString(36).substr(2, 9);
  }, []);
  ...
  const connectToAgent = useCallback(async (config: CampaignAgentConfig) => {
    try {
      ...
      const participantName = 'campaign_session_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9)
      ...
      // Обрабатываем аудио треки от агента
      ...
      addMessage('Вы подключены к агенту кампании. Микрофон автоматически включен для голосового общения!', 'bot');
